...
""",
            ...
            if bool(getattr(cfg, "threshold_exit_enabled", False)):
                ...
                lifecycle.notes = f"threshold_exit_active qty={filled_qty}"
                entry_px_dbg = None
                ...
                body = _threshold_exit_state_block(
                    ...
                    gain_pct=None,
                    loss_pct_now=None,
                    gain_trigger_pct=float(getattr(cfg, "threshold_exit_gain_pct", 1.0) or 1.0),
                    loss_trigger_pct=float(getattr(cfg, "threshold_exit_loss_pct", 0.7) or 0.7),
...
""",
        ...
        try:
            if lifecycle.stage == "IN_POSITION" and bool(getattr(cfg, "threshold_exit_enabled", False)):
                entry_px = None
                ...
                if (shared_last_px is None) or (not entry_px) or (entry_px <= 0):
                    ...
                    _append_note(lifecycle, f"THRESHOLD_EXIT_WAITING_FOR_PRICE last={shared_last_px} basis={entry_px}")
                    ...
                    _append_note(lifecycle, f"THRESHOLD_EXIT_EVAL last={shared_last_px} basis={entry_px} source={profit_basis_source} gain={gain_pct:.2f}% loss={loss_pct_now:.2f}% gain_trig={gain_trigger_pct:.2f}% loss_trig={loss_trigger_pct:.2f}%")
                    exit_reason = None
        ...
        try:
            if lifecycle.stage == "IN_POSITION" and bool(getattr(cfg, "enable_time_profit_capture", False)) and not bool(getattr(cfg, "threshold_exit_enabled", False)):
